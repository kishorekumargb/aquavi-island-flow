...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log("User verified with role:", roleData[0].role, "- processing delivery confirmation");
    ...
    const itemsHtml = items.map((item: any) => {
      ...
      return `
                              <tr>
                                <td style="padding: 12px 16px; border-bottom: 1px solid #e5e7eb; color: #374151; font-size: 14px;">${displayName}</td>
